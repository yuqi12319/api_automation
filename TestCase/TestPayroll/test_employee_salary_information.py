...
class TestEmployeeSalaryInformation():
    ...
    @pytest.mark.parametrize('data',YamlHandle().read_yaml(
        'Payroll/EmployeeSalaryInformation/employee_salary_information.yaml'))
    def test_employee_salary_employee_information(self, data):
        response = EmployeeSalaryInformation().get_employee_salary_information_api(self.url_path, data)
