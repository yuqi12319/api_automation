...
class TestApprovalSetting:
    ...
    @pytest.mark.parametrize("data", YamlHandle().read_yaml('SingleInterfaceData/Approval/team_leave_approval_setting.yaml'))
    ...
    @pytest.mark.parametrize("data", YamlHandle().read_yaml('SingleInterfaceData/Approval/get_team_leave_approval_list.yaml'))
    def test_get_team_leave_approval_list(self, data):
        ...
        if num is None:
            ...
            YamlHandle().write_yaml('SingleInterfaceData/Approval/delete_team_leave_approval.yaml', 'url', '/dukang-workflow/api/workflow_setting/'+w_id)
            Assertions().assert_mode(res, data)
        else:
            print("团队休假列表为空，请检查")
