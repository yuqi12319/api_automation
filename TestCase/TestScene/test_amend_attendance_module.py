...
class TestAmendAttendance:
    ...
    def test_main_scene(self, data, setup_class):
        with allure.step('第一步：获取补卡默认审批流,修改审批人为指定人，自动审批通过'):
            # 获取默认加班审批流id
            data['amend_attendance_approval_list']['params']['company_id'] = setup_class[1]
            amend_attendance_approval_list_res = WorkflowSetApi(setup_class[0]).attendance_approval_list_api(
                data['amend_attendance_approval_list'])
            Assertions().assert_mode(amend_attendance_approval_list_res, data['amend_attendance_approval_list'])
            for item in amend_attendance_approval_list_res.json()['data']:
                if item['name'] == '默认补卡审批流':
                    default_amend_attendance_approval_id = item['id']
                else:
                    MyLog().debug('当前公司无默认补卡审批流')

            # 修改外出默认审批流，自动审批通过
            data['update_amend_attendance_approval']['workflow_setting_id'] = default_amend_attendance_approval_id
            data['update_amend_attendance_approval']['body']['approverlist'][0]['employee_id'] = setup_class[2]
            data['update_amend_attendance_approval']['body']['company_id'] = setup_class[1]
            data['update_amend_attendance_approval']['body']['orglist'][0]['co_org_id'] = setup_class[1]
            update_amend_attendance_approval_res = WorkflowSetApi(setup_class[0]).update_attendance_approval(
                data['update_amend_attendance_approval'])
            Assertions().assert_mode(update_amend_attendance_approval_res, data['update_amend_attendance_approval'])
        ...
        with allure.step('第三步：申请补卡'):
            pass


if __name__ == '__main__':
    pytest.main(['-sv', 'test_amend_attendance_module.py', '--env', 'test1'])
