...
class Request:
    ...
    def get_requests(self,url,data=None,headers=None,cookies=None,timeout=TIMEOUT):
        ...
        try:
            response = requests.get(url=url,params=data,headers=headers,cookies=cookies,timeout=timeout,verify=False)
        ...
        if self.is_json(response.text):
            self.log.info("URL: %s \n[REQUEST BODY]:%s \n[RESPONSE]:%s \n[CODE]: %s" % (url, response.request.body, response.json(), response.status_code))
        else:
            self.log.info("URL: %s \n[REQUEST BODY]:%s \n[RESPONSE EMPTY] \n[CODE]: %s" % (url, response.request.body, response.status_code))
