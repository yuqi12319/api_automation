...
class Request:
    ...
    def post_requests(self, url, params=None, json=None, headers=None, cookies=None, timeout=TIMEOUT):
        ...
        try:
            response = requests.post(url=url, params=params, json=json, headers=headers, cookies=cookies, timeout=timeout,
                                     verify=False)
        except requests.exceptions.ConnectTimeout:
            raise Exception("CONNECTION_TIMEOUT")
        except requests.exceptions.ConnectionError:
    ...
    def send_request_method(self, method, url, params=None, json=None, headers=None, timeout=TIMEOUT):
        if headers is None:
            headers = {
        ...
        if method in ['get', 'GET']:
            ...
            response = self.put_requests(url=url,params=params, json=json, headers=headers)
        else:
