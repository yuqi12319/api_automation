...
def update_idcard():
    ...
    for employee_id in employee_ids:
        ...
        update_sql = "UPDATE employee_certificate SET type = 'IDCard',number=" + str(
            id_card) + ",photo='[]' WHERE employee_id = " + str(employee_id['id'])
...
def relevance_employee_salaryproject():
    ...
    for relevance_employee_data in relevance_employee_datas:
        url = "http://dktest2-workio.bipocloud.com/services/dukang-payroll/employee/salary?employeeId=" + str(
            relevance_employee_data['id'])
        data = {
            "baseSalary": [
                {
...
def get_employee_ids(database, attendance_group_id):
    select_sql = "SELECT employee_id FROM attendance_group_employee_map WHERE attendance_group_id=" \
                 + str(attendance_group_id)
    query_result = mysql_operate_select_fetchall(database, select_sql)
    list_result = []
    for result in query_result:
        list_result.append(result['employee_id'])
    return list_result
