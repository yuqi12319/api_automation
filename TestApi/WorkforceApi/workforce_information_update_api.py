...
class WorkforceInformationUpdateApi(Const):
    ...
    # 获取用工更新申请员工职位信息
    def get_update_employee_position_api(self, data):
        url = self.url_path + '/dukang-workforce/api/workforce_update_records/employee_position'
        res = self.request.send_request_method('get', url=url, params=data['params'], headers=self.headers)
        return res

    # 新增用工职位信息更新
    def post_update_employee_position_api(self, data):
        url = self.url_path + '/dukang-workforce/api/workforce_update_records/employee_position'
        res = self.request.send_request_method('post', url=url, json=data['body'], headers=self.headers)
        return res
